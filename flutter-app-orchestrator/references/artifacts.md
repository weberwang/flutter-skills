...
| 任务需要持久化独立验收 | `docs/tasks/<task-id>/review.md` | F0 证据引用、F1 分诊与通道、F2 各通道的覆盖范围/快照/结论/发现、F3 收敛结果 |
...
- F1 分诊、Product/QA/技术/视觉/Release 通道与 F3 收敛写入同一 `review.md` 的具名小节；每个 F2 小节保留作者、覆盖范围、快照和结论，未触发通道不写 `N/A`。
