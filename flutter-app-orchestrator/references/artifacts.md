...
| `high`、`release` 或并发多代理任务 | `.codex-workflow/tasks/<task-id>.yaml` | 风险、验证、候选提交、租约、分支、范围、验收和合并记录 |
...
未满足条件时不创建占位文件，也不写 `N/A` 文档。轻量和标准顺序任务不创建任务状态、进度账本条目或自动收尾记录。
...
- `.codex-workflow/progress.md` 只索引受控高风险、发布或并发任务；以任务状态、页面决策和审阅记录为真相来源。
