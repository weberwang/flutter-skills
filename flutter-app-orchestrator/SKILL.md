...
2. Draft global UX/UI goals, flows, states, navigation model, and design-system direction with `flutter-ux-ui-quality`. Use [references/global-visual-direction-prompt-template.md](references/global-visual-direction-prompt-template.md) to create and save `docs/design/prompts/global-visual-direction-prompt.md`, which defines the three-direction comparison and its PRD alignment. When the exploration uses a representative page to generate effect images, do not use this global template as the image-generation prompt. Instead, use [references/page-hifi-mockup-prompt-template.md](references/page-hifi-mockup-prompt-template.md) to create and save one formal prompt per candidate direction at `docs/design/prompts/global-visual-directions/<direction-name>-representative-page-hifi-mockup-prompt.md`, then use those saved page prompts to generate the comparable images. Keep the representative page, device, information structure, copy, data, and state identical across all directions; vary only the approved visual-direction fields. Trace every product claim, task, state, content constraint, and acceptance criterion to the PRD artifacts; document the product-design-principle check in each formal prompt file. Run `@product-design user-context` preflight followed by `@product-design get-context` and `@product-design ideate` to select one direction.
...
- No representative-page effect-image generation during global visual exploration before each candidate direction has its own saved page-level prompt at `docs/design/prompts/global-visual-directions/<direction-name>-representative-page-hifi-mockup-prompt.md`, populated from the page high-fidelity prompt template with identical non-visual page content across candidates.
