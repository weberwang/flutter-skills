...
1.1. After the PRD artifacts are complete, conduct market analysis and save `docs/product/market-analysis.md` before global visual direction positioning.
...
2. Draft global UX/UI goals, flows, states, navigation model, and design-system direction with `flutter-ux-ui-quality`. Use [references/global-visual-direction-prompt-template.md](references/global-visual-direction-prompt-template.md) transiently to define exactly three market-informed visual-system direction positionings that satisfy the active preset's direction mix. Each direction must define product character, color roles, typography, shape language, imagery and icon style, material, motion, restatable signature, design-system extension rules, expected Flutter implementation path, cost, and risk. Do not generate any representative-page, module, page, or screen effect image during this stage; do not call `flutter-hifi-mockup`, do not use the page prompt template, and do not create `.codex-workflow/visuals/global/`. List the three direction definitions and wait for the user to choose one; do not select automatically.
...
7. Execute modules with `flutter-subagent-delivery`. When a module first becomes eligible, re-enter `grilling` to confirm included functions, non-goals, page/state boundaries, dependencies, and acceptance path; after explicit confirmation, create `docs/plans/modules/<module-name>-scope.md` and refine module functions, page functions, states, contracts, criteria, and task briefs. For each UI module, then run the Module Effect-Image Interrogation Gate once before any page effect image: confirm the module's visual outcome, pages and states requiring effect images, page budgets, signature expectations, accepted custom-component/motion/bitmap/maintenance cost, and shared understanding that the images cannot change scope. Record it in `docs/product/grilling-log.md` and the module scope. Do not repeat it for every page unless a new decision or conflict appears. Each page task must then create a low-fidelity Pencil structure, pass Wireframe Review, and convert it into text specs before `flutter-hifi-mockup` generates exactly three page-level candidates. Review the candidates with exactly one Effect Image Reviewer, wait for explicit selection and change disposition, then persist the exact selected image first under `.codex-workflow/visuals/pages/<page-name>/` before writing the page prompt, brief, design freeze, and progress record.
8. Use the selected page-level high-fidelity effect image and page design freeze as the sole visual source of truth for Pencil high-fidelity restoration; the wireframe remains a scope, structure, state, and interaction prerequisite only. Apply [bitmap-decomposition-standard.md](../flutter-pencil-design/references/bitmap-decomposition-standard.md): first split runtime data from its renderer and fixed treatment, then classify every atomic unit as bitmap, UI, or data. Never crop, generate, export, or bundle representative runtime data as a production bitmap. Run a second back-to-front and top-left-to-bottom-right sweep that explicitly covers backgrounds, edge and corner decoration, overlays, textures, every icon placement/state, logos, and clipped fragments; record a coverage audit with zero unowned elements, zero data-derived export candidates, and no missing decoration or icon. Review every fixed visual resource for a separate production verdict and 100% match evidence; otherwise assign `generate` and complete the asset-atlas flow. Record data placeholders plus their Flutter renderer, loading state, and fallback. For every bitmap, record its source, crop, size, background treatment, and target node. For every UI unit, record the native-Flutter feasibility reason and expected implementation approach; use a bitmap fill only when Flutter cannot reproduce it exactly. Record unresolved visual facts instead of guessing.
9. After the analysis identifies a bitmap or bitmap-fill requirement, use `flutter-asset-atlas` for its reuse check and production planning. Present the complete pre-slicing confirmation table inline, wait for explicit user confirmation, and record its version and confirmation time before any generation, adaptation, extraction, export, transparentization, or slicing. After confirmation, complete production, inventory, and fidelity review. Reconfirm affected rows after any material change. New bitmap generation must use product-design or image generation tools by default and follow the global design freeze and page design freeze. Treat bitmap enhancement as a design change: replace the corresponding asset in the design draft, preserve the approved layout and constraints, and record the updated design evidence before implementation. When the analysis identifies no bitmap or bitmap fill, record `N/A: no bitmap or exported visual assets` in the task brief and `.codex-workflow/progress.md`.
...
- Treating the global direction freeze as a substitute for module effect-image interrogation, low-fidelity structure, Wireframe Review, or page mockup approval.
