...
- 只有多个分支需要同时写入、需要保护现有脏工作区，或 Controller 必须常驻集成分支时才使用 `worktree` 隔离。
...
活动状态文件在任务分支创建后由 Controller 保持未提交；任务分支的提交历史不得携带它。自动收尾前，Controller 工作目录除当前任务状态文件外必须干净。
...
- 所有必需审查批准最终候选后，Controller 仅运行一次 `finalize-task.py <state> --repository <工作目录> --integration-branch <分支>`。脚本检查状态、候选提交、报告、租约、基线和 `git diff --check`；branch 模式安全切回集成分支、合并并删除任务分支，worktree 模式额外验证并删除 worktree；最后只提交接受状态。
