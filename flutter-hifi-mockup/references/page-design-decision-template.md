# 页面设计决策

每个 UI 页面只维护一份 `docs/design/pages/<page-name>/design-decision.md`。候选、完整规划表和评审草稿保持在对话中，不写入仓库。

## 页面与语义契约

- 页面 / 状态 / 模块：
- 线框级别：Full / Lightweight / Reuse；原因：
- 必需内容、状态、交互与无障碍：
- 线框审阅人 / 快照 / 结论：

## 布局与自适应契约

> 这里冻结跨视口的关系和行为，不无必要地冻结绝对坐标、卡片像素尺寸、最终留白或图文裁切。低保真图只证明语义结构；高保真图也不能替代本契约。权威规则：`$flutter-ux-ui-quality` 技能中的 `references/responsive-layout-strategy.md`。

- 响应式契约路径 / 版本（必须与权威文件一致）：
- 契约完整性：区域树 / 布局实现映射 / 结构断点 / 证据矩阵均已填写；映射覆盖率：`100%` / 未完成（阻塞）：
- 基准画布坐标声明：仅作视觉参考；禁止将 `x/y/left/top/right/bottom` 或比例坐标直译进 Flutter：
- 页面区域树（区域 ID、父子层级、兄弟顺序、内容/数据/固定资产所有权）：
- 布局实现映射（区域 ID → 父约束 → Flutter 原语 → 尺寸模式 → 窄/宽回退）：
- 目标平台、方向与视口范围：
- 使用的布局分类 / 断点触发条件（每个断点的“条件 → 结构变化 → 回退”）：
- 具体结构断点证据（每个断点两侧的逻辑视口、方向、文字比例、状态）：
- 内容最大宽度 / 外侧 inset / 列数 / 最小列宽 / gutter：
- 语义锚点与约束关系（对齐、间距 token、伸缩、换行、折叠）：
- 导航模式及断点间结构变化（路由、选中状态和任务语义保持规则）：
- 文档流区域：
- 停靠元素：固定 / 吸顶 / 悬浮；视口锚点、显隐、层级、内容避让和命中区：
- 允许叠层白名单（区域、中文原因、边界、命中区、遮挡 padding、无叠层回退）：
- 每个滚动轴的唯一所有者；有意嵌套滚动的原因与手势/焦点规则：
- SafeArea、系统栏、手势区、键盘、折叠铰链/分屏避让：
- 大字体、长文本、本地化/RTL、横屏和溢出策略：
- ScreenUtil 仅用于根初始化和共享 token；约束驱动实现边界：
- 具体视口证据矩阵（逻辑宽×高、方向、文字比例、状态、键盘/inset、预期结构、Widget/Golden/审计命令）与通过条件：

## 冻结

- 全局方向：
- 冻结图路径 / 候选 ID / SHA-256 / 尺寸：
- 用户确认时间与变更处置：
- 精简生图提示词哈希：
- 必须遵守的视觉约束与允许偏差：

## 资产与 Pencil

- 分解覆盖审计与未决事实：
- 资产清单（存在固定视觉资产时）：`asset-manifest.md`
- Pencil：Required / Not required；原因：
- `app-design.pen` 页面 / 节点 ID：
- Flutter 交接约束：
- Pencil 交接输出：区域树、父子/兄弟约束、相对锚点、尺寸模式、流式/叠层边界、断点变化、Flutter 原语建议、禁止坐标直译、未决事实：
- Pencil 契约完整性：完整 / `NEEDS_CONTEXT`（未完成不得交接）：

## 验收

- 截图或 golden 路径：
- 响应式 Widget / Golden / 审计命令及结果：
- 已知偏差与接受人：
