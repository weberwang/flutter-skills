...
`flutter-workflow` 是 Flutter 技能链的总编排器。它负责维护唯一 workflow state，锁定当前阶段、下一个 skill、允许的状态变更，并阻止任何绕过门禁的推进。必要时可以把这些状态落成运行时产物，但它们不属于 skill 的稳定资源。
...
15. 高质量品牌视觉提示词应先抽取公共品牌世界约束，再进入下游工作流；不要把品牌稿件里的项目私有名词直接硬编码到下游工作流。
...
已生成 1 张代表页本地草图，正在等待你确认或提出调整。手动模式会在这里等待确认；`--auto --preview` 可以继续自动推进到代表页最终效果图。

### `global_effect_images_ready`

代表页草图已确认，代表页最终效果图与所有范围内页面的浅色效果图都已生成并批准。只有到这里，才允许进入 Stitch 结构化设计源阶段。

这里新增一条默认伴生约束：

1. 每张共享效果图生成时都要同步生成一份 `sheet atlas`
2. atlas 必须使用透明背景
3. atlas 只包含 UI 层，不包含数据层
4. atlas 必须附带可切割 manifest，供后续切片与资源确认使用
5. 如果 atlas 已经是透明背景，透明背景禁止再处理
6. 只有纯色背景才允许处理为透明；渐变、纹理、图片等非纯色背景禁止处理为透明
7. atlas 的 cell 图片不得随意补局部背景底板，也不得额外画线框、描边框或调试框
...
冻结设计已经被翻译成 Flutter 可实现的 token、组件边界、屏幕结构、位图/原生绘制决策、显示层决策表，以及页面级 `display_restoration_blueprint`。它不再承担“触发项目初始化”的职责，而是作为进入模块实现前的最后技术准备。

这里新增的 `display_restoration_blueprint` 不是新阶段，而是架构阶段必须落出来的实现蓝图。它至少要锁定：

1. region 到 widget/container 结构的映射
2. scroll/list/sticky/overlay 的归属和语义
3. 关键状态在页面里的可见槽位与替换边界
4. 必须复用的导出资源和资源绑定关系
5. fidelity-critical 区域的 parity 检查点
...
3. 先确认当前模块的 `impl.md`、模块效果图、HTML 交互原型和架构产物都已冻结
4. 先把冻结后的 HTML 交互原型转译成页面级 `display_restoration_blueprint`
5. 按 `display_restoration_blueprint` 先落非显示层契约、状态和数据流
6. 再按同一份 `display_restoration_blueprint` 还原 Flutter 显示层，默认不同时打开多个模块实现通道

这里的关键不是“先写哪层代码”，而是“先冻结哪份实现约束”：

1. 交互定义不能留到显示层实现时再猜
2. `display_restoration_blueprint` 必须先锁定 region 到 widget 结构、scroll/list/overlay 语义、状态槽位、资源绑定和 parity 检查点
3. 页面不得靠手动 `bottom padding` 兜底制造底部空间；底部留白只能来自 `SafeArea`、冻结的 `bottom action area`、冻结的 `sticky footer`，或蓝图里显式批准的 scroll content inset
4. 非显示层代码如果与这份蓝图冲突，应回修非显示层或回到设计源控制，不允许让显示层被动迁就代码

显示层实现前，如果存在对应页面效果图，必须先通过 `$image-to-code` 检查页面图。Stitch 还原时允许下载批准后的图片资产并直接作为项目资源使用，不强制再生。

模块实现期的页面效果图现在也要同步生成一份 `sheet atlas`，并遵守同一套约束：

1. atlas 必须是透明背景
2. atlas 只包含 UI 层，不包含数据层
3. atlas 必须附带可切割 manifest
4. atlas 只是后续独立资源切片基线，不替代逐资源确认与生成
5. atlas 的 cell 图片不得随意补局部背景底板，也不得额外画线框、描边框或调试框

### `module_done`

模块代码已落地，并且已经先通过 design parity 检查，再完成最终人工视觉检查。

这里要额外明确：

1. app 页面最终验收只能放在整个工作流最后
2. 最终验收必须由人工验收完成
3. `@product-design design-qa` 只是阻断性辅助检查，不是最终验收人
...
2. 推进到全局视觉设计头脑风暴
3. 在你尚未确认最终设计方向时停下
4. 你确认后，如果启用了 `--preview`，先自动生成代表页草图
5. 代表页草图确认后，如果启用了 `--preview`，自动生成代表页最终效果图并继续生成剩余页面效果图而不在普通确认点停下
...
1. 尽量把多个连续确认收敛成一个真正需要你拍板的确认。
2. `--full-auto` 会继续自动吃掉“只剩一个默认选项”的人工门禁，例如首张方向图、首张模块最终效果图这类只是在验证既定方向的确认。
3. 仍然必须停下的确认只有：方向分叉、Stitch 项目绑定、candidate_reuse、最终人工验收。
4. 人工负责拍板，但工作流必须先给出多个推荐选择。
5. 不要让人工从空白开始定义风格、品牌张力或状态视觉语言。
6. 人工决策节点统一输出：decision_topic、recommended_options、fit_reason、risks、default_recommendation、human_action_required。
7. 推荐项至少包含：id、title、summary、fit_reason、risks、recommendation_strength。
...
### 为什么要同步生成 sheet atlas

单张效果图更适合确认整体方向，但不适合直接拿来切稳定资源。

同步生成 `sheet atlas` 的原因是：

1. 先把稳定 UI 壳层和数据层拆开，避免误把运行时内容当静态资产
2. atlas 统一使用透明背景，方便后续切片复用到不同运行时表面
3. atlas 附带 manifest 后，后续资源流能更稳定地判断哪些区域该切、哪些区域该保留为 `flutter_native` 或占位
4. atlas cell 保持素材本体即可，避免后补背景或线框污染后续切片边界

这里的 atlas 只是伴生产物，不是最终资源包：

1. 它不替代独立 bitmap asset
2. 它不跳过页面级 bitmap list 确认
3. 它不允许把多个最终资源永久打包成一个运行时大图
...
### 为什么还要增加 `display_restoration_blueprint`

因为“HTML 交互原型已经很准”并不等于“Flutter 页面会自动跟着准”。

如果没有这层蓝图，工程实现时仍然会出现二次解释：

1. HTML 原型表达的是结构化设计源
2. Flutter 代码表达的是具体 widget 树、状态槽位和交互归属
3. 中间如果只靠开发者理解，scroll、overlay、层级、资源绑定和状态区就容易漂

所以现在要求先把 HTML 原型和架构产物收敛成一份 `display_restoration_blueprint`，再让非显示层和显示层都围绕这份蓝图落地，而不是各自理解一版。

### HTML 原型默认技术栈怎么选

当前默认推荐的 HTML 原型技术栈是 `Vite + React + TypeScript`。

原因不是它“更潮”，而是它对 Flutter restoration 更稳：

1. 原型实现速度足够快，页面拆分和交互组织成本低
2. React 组件树更容易回读成 region、状态槽位、overlay 归属和页面结构语义
3. TypeScript 更适合把临时原型交互收敛成后续 `display_restoration_blueprint` 可消费的结构化约束

对比另外两种常见方案：

1. `Vue`
说明：能做，也顺手，但在当前仓库默认流程里没有额外收益，反而会引入第二套组件心智模型。除非项目已明确选 Vue，否则不作为默认栈。
2. `HTML/CSS/JS`
说明：起步最快，但最不利于后续 Flutter restoration。结构、状态和交互语义容易散落在选择器和临时脚本里，冻结后很难稳定回译。

所以当前规则是：

1. 默认使用 `Vite + React + TypeScript`
2. 不默认使用 `Vue`
3. 不默认使用纯 `HTML/CSS/JS`

### 为什么单张页面效果图不够

单张主效果图通常只能锁住主态，不足以锁住状态视觉语言。

如果没有补充关键状态的视觉证据，后续实现很容易退化成“一个小卡片 + 一段说明文字”的保守方案，结果虽然功能上没错，但视觉语言会明显偏离主流产品设计。

这里默认要避免两种退化：

1. 空状态、错误态、权限态只靠大段解释性文字撑页面
2. 关键状态直接复用主态结构，再临时塞一个说明卡片了事

更推荐的状态视觉语言是：

1. 优先用插画、图形、占位结构、品牌元素、比例和留白来表达状态
2. 文案只做辅助说明，不要让大段文字成为页面主体
3. 状态切换时允许页面结构发生明确变化，而不是所有状态都套同一个主态容器
...
1. 模块冻结前先检查高保真视觉契约
2. `flutter-uiux-to-architecture` 必须产出 region 级决策和页面级 `display_restoration_blueprint`
3. 显示层实现前跑 display preflight，并明确记录检查回执
4. 对应页面图存在时，先走 `$image-to-code`
5. 代码和截图出来后，先做 design parity 检查，再交给人工视觉检查

这里要特别强调：

1. 后置的是“显示层代码实现”，不是“交互定义”
2. 交互定义、页面结构语义和资源绑定必须在 `display_restoration_blueprint` 里先冻结
3. 页面级底部空间必须先冻结归属，不允许通过页面手动 `bottom padding` 临时补空白
4. 关键状态不能只靠单张主效果图脑补，必须补状态视觉证据或冻结后的状态视觉描述
5. 空状态、错误态、权限态应优先做视觉语言设计，尽量少用说明性长文案，多用图形、插画、比例、留白和 CTA 组织页面
6. 如果非显示层实现和蓝图冲突，应该修非显示层或回退设计控制，不应该改薄显示层来迁就代码
...
1. 需求到 PRD：`flutter-workflow` 内部 requirements-to-PRD 流
...
14. 页面级显示层蓝图：由 `flutter-uiux-to-architecture` 产出 `display_restoration_blueprint`
15. 模块实现：`@superpowers` -> `Spec` -> `Plan` -> 先按蓝图落非显示层，再按同一蓝图还原显示层
