...
背景处理资格只负责回答“是否允许继续做背景转透明处理”，不改变 atlas 主图最终仍应为透明背景这一要求。

门禁规则固定为：

1. 如果输入已经是透明背景，禁止再处理背景
2. 只有在输入是纯色背景时，才允许处理背景为透明
3. 如果输入不是透明背景且也不是纯色背景，禁止处理背景为透明
...
10. `source_page`
11. `background_state`
12. `background_processing_allowed`
...
4. `background_state` 至少支持：
   - `transparent`
   - `solid_color`
   - `non_solid`
5. `background_processing_allowed` 只有在 `background_state=solid_color` 时才允许是 `true`
...
此外，atlas 和后续切片链路还必须先通过背景处理资格门禁：
