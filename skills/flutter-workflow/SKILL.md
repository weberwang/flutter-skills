...
<!-- 中文说明：这里只替换“新增补充位图的生图入口”，atlas 去背景仍然保留 `$imagegen`，因为那一步是透明化处理，不是重新生成位图资源。 -->
- Every new supplemental bitmap resource that is still required beyond the accepted atlas-slice outputs must be generated through `Product Design:ideate` as its own prompt and saved as its own file. When an accepted `UI-only sheet atlas` already serves as the standard runtime resource atlas for that page, its confirmed slice outputs may be used directly as final runtime assets.
