...
description: Use when coordinating a Flutter product workflow across idea intake, requirements, PRD, technical baseline, Product Design brief, module responsibility confirmation, reference-image effect images, UI atlas preparation, tool-selected design source generation, shared/module design freeze, module impl docs, native HTML prototype confirmation, design-source-to-Flutter restoration, architecture, display restoration blueprint, implementation readiness, QA, human visual inspection, workflow state, or when the user asks what stage should happen next.
---
...
Route a Flutter module through rough idea intake -> idea-sketch brainstorming -> requirements brainstorming -> companion page-navigation-flow output -> page-navigation-flow confirmation -> PRD document generation and refinement from confirmed navigation -> global technical baseline -> target design-device preset, design viewport, and image output scale confirmation -> `@product-design` brief confirmation -> Mobbin-backed global theme and style scheme selection -> `DESIGN.md` output for theme, hierarchy, public shell, component family, and interaction principles only -> shared freeze -> early project initialization plus bootstrap preparation -> module responsibility and primary-page scope confirmation -> mandatory module final effect-image confirmation -> module `impl.md` generation under the frozen design, confirmed module scope, and confirmed effect image -> mandatory native HTML module prototype restoration from that confirmed effect image -> mandatory module design source confirmation -> module design freeze -> implementation RD readiness -> architecture -> display restoration blueprint readiness -> implementation workflow -> Flutter render versus frozen-design QA -> `@product-design` design QA -> human visual inspection -> later atlas preparation, atlas slicing, tool-specific design-source reinforcement, and bitmap assets only when the first-pass implementation plus implementation review still prove they are necessary.
...
- The global stage may include one `global_style_experience_image` to make the selected style direction visually reviewable. It must be a non-page visual mood preview for theme, hierarchy, density, surface depth, and component feel, with `non_page_design_evidence=true`; it must not show a complete app screen, page flow, final layout, exportable asset atlas, or tool-specific design-source-ready page design.
- The global stage must not generate page-level HTML prototypes, representative page sketches, final effect images, page-level tool-specific design-source files, atlas bundles, or shared page design drafts. Any older shared/global wording that requests those artifacts is superseded by this boundary.
- Page design belongs to the module stage after the active module scope packet fixes function, the primary task path, and behavior boundaries for the confirmed primary page. The default planning target is the primary page only, and additional page states may be appended later only after explicit human confirmation. Only then may the workflow generate module effect images. The executable module `impl.md`, module HTML prototype, and module design source must then be derived from that confirmed page baseline. Atlas evidence and bitmap assets stay out of the first-pass implementation path unless the later visual-enhancement branch is explicitly opened and handed to `$flutter-visual-enhancement-branch`.
...
- Forbidden-as-required shared/global artifacts:
  shared/global page-level HTML prototypes, page-sketch drafts, final effect images, atlas bundles, tool-specific design-source files, and shared page comps must not become required inputs for the current shared/global freeze.
...
- Required module-stage artifacts:
  for the initial implementation pass, the required path is confirmed module responsibility and primary-page scope, confirmed module final effect image, executable module `impl.md`, confirmed native `HTML/CSS/JS` prototype restored from that effect image, mandatory selected-tool design source, architecture output, and display restoration blueprint. Atlas and standalone bitmap generation remain conditional follow-up evidence only when the first-pass implementation plus implementation review still show regions that code restoration alone cannot cover faithfully enough.
- Conditional module artifacts:
  `display_evidence_pack_ready` is required only when the page is fidelity-sensitive enough that later restoration would otherwise have to guess. Additional standalone bitmap assets are required only when atlas slices and code restoration are still insufficient after design-source review.
...
## Visual Enhancement Branch Handoff

The later visual-enhancement branch is now owned by `$flutter-visual-enhancement-branch`, not by `flutter-workflow`.

- `flutter-workflow` may decide when the branch is allowed to open, but once it opens, branch-local sequencing must be delegated to `$flutter-visual-enhancement-branch`.
- Use that handoff only after all target modules have already completed the first-pass functional implementation loop and review still proves that native restoration is insufficient.
- Before handing off, `flutter-workflow` must explicitly mark which later page modules or module pages need the visual-enhancement branch, and it must record that scope plus the reason in the workflow record instead of leaving the branch target implicit.
- `$flutter-visual-enhancement-branch` owns atlas extraction analysis, solid-background atlas generation through `Product Design:ideate`, optional `$imagegen` background removal, deterministic `ui-sheet-atlas-slicer`, `display_evidence_pack_ready`, tool-specific design-source reinforcement, global asset-catalog reuse checks against `docs/project/assets/global-asset-catalog.json`, post-review bitmap list confirmation, and standalone supplemental bitmap generation through `Product Design:ideate`.
- When the selected `design_source_branch` is `stitch_design_source_branch`, the later visual-enhancement branch must also execute Stitch-based design-source reinforcement on that same branch. Reuse the recorded `stitch_project_mode` plus `design_source_project_ref`, and when the project already exists, continue in the same recorded Stitch project `id` instead of creating a new project during reinforcement.
- `flutter-workflow` should only persist branch-local stage updates such as `ui_sheet_atlas_analysis_ready`, `ui_sheet_atlas_ready`, `ui_sheet_atlas_transparent_ready`, `ui_sheet_atlas_slices_ready`, `display_evidence_pack_ready`, `module_asset_resources_ready`, and the refreshed `design_restoration_ready`.
- The workflow record should keep one explicit later-branch mark per active module row, naming the marked module pages, their branch reason, and whether the mark is still `not_required`, `candidate`, or `required`.
- The asset catalog remains the mandatory generated-image record table. Every workflow-generated final effect image, matching atlas files plus manifests, and approved bitmap asset that enters downstream consumption must still be written into `docs/project/assets/global-asset-catalog.json` in the same workflow step.
- Every workflow-generated image that enters evidence or downstream consumption must still leave one manual image review result before later confirmation or freeze work continues.

## Design Source Branches

The module design-source stage must lock exactly one explicit design-source branch for each active module before design-source generation starts. Supported values are `stitch_design_source_branch` and `pencil_mcp_design_source_branch`.

- `stitch_design_source_branch` maps to `design_source_type=stitch` and must execute through Stitch MCP with `model=GEMINI_3_1_PRO`. Before execution, it must also record `stitch_project_mode=new_project|existing_project` plus the corresponding `design_source_project_ref`. When the target project already exists, record its Stitch project `id`, use `stitch_project_mode=existing_project`, and continue in that project instead of creating a new one by default. Use the official Stitch MCP reference as the capability boundary for this branch.
- `pencil_mcp_design_source_branch` maps to `design_source_type=pencil_mcp` and must execute through Pencil MCP. Do not route this Pencil branch through CLI or ad hoc file generation in this workflow.
...
- Before `design_source_ready` or `design_restoration_ready` may be promoted, the workflow record must already index both `design_source_branch` and `design_source_type`. When the selected branch is `stitch_design_source_branch`, it must also index both `stitch_project_mode` and `design_source_project_ref`, and for `existing_project` that reference must contain the target Stitch project `id`. The record must then capture the concrete tool-specific file path, export path, or packet reference for the accepted artifact.

## Final Effect Image Boundary

The module final effect-image stage exists after module responsibility confirmation and before executable module-document generation. It is the required high-fidelity visual baseline for the active module's confirmed primary page and must be generated and accepted before the workflow asks for the executable `impl.md` and before the workflow asks native `HTML/CSS/JS` to restore that page.

- Use `Product Design:ideate` to generate the module final effect image after the active module scope packet already fixes function, the primary task path, and non-display behavior boundaries for the confirmed primary page.
...
- Treat the final effect image as the pre-implementation visual baseline for later `impl.md`, HTML, design source, architecture, and Flutter restoration. Do not treat it as a late polish branch.
- The native HTML prototype, later design source, and later Flutter display restoration must inherit the confirmed effect image's page structure, hierarchy, grouping, density, action order, and visible emphasis unless the workflow explicitly reopens the image direction.
...
- If review feedback is still about page purpose, module order, emphasis, or overall visual direction, revise the same module final effect-image direction pass instead of opening a separate sketch gate or pushing the correction burden to the later prototype.
- Atlas preparation, slicing, design-source reinforcement, and standalone bitmap generation remain later follow-up work only when the confirmed effect image and restoration review still prove they are necessary.
...
The `display_evidence_pack_ready` stage exists to prove that fidelity-critical regions have enough evidence before the optional post-implementation visual-enhancement path proceeds. Inside that later path, the stage is managed by `$flutter-visual-enhancement-branch`.
...
- The display evidence pack must be confirmed before the workflow finalizes any later effect-image, atlas, slicing, design-source, or bitmap-asset refinement pass, but it is not a blocker for the first-pass standard-library implementation path.

## Module Native HTML Prototype Boundary

The `module_html_prototype_ready` stage exists to make the active module reviewable in native web form by restoring the already confirmed module final effect image before the workflow commits to architecture and first-pass implementation.

- Enter this stage only after the shared/global design freeze is complete, the active module scope packet has already fixed the module function, the primary task path, and non-display behavior boundaries for the confirmed primary page, the matching module final effect image has already been explicitly confirmed, and the executable module `impl.md` has already been generated from that confirmed baseline.
- Before the first prototype draft is created, verify that the active module already carries a commercial-product creation packet: `surface_goal=commercial_product_app`, exactly one `primary_task`, exactly one dominant `primary_cta`, explicit `first_screen_required_information`, explicit `defer_or_collapse_content`, explicit `information_density_posture`, `copy_posture=minimal`, explicit `disallowed_surface_modes`, `target_user_maturity`, `price_confidence_level`, `competitive_reference_band`, `trust_signal_strategy`, and `visual_premium_keywords`. If any field is missing, stop and repair the brief or `impl.md` before creating the prototype.
- Before the first prototype draft is created, verify that the active module `impl.md` also carries a page-content contract for the confirmed primary page and every already-required page-local state. Keep that contract inside the executable `impl.md`; do not create a separate module UI/UX sidecar document for the same responsibility.
- At minimum, the page-content contract must state what must be visible on the default first screen, what must be deferred, collapsed, paged, or moved to a secondary surface, what must stay off the default surface, which action is the dominant `primary_cta`, which secondary actions may remain visible, and how required `empty`, `loading`, `error`, permission, or other confirmed states change the visible content.
- Before the first prototype draft is created, verify that the confirmed module final effect image is already indexed as the prototype's mandatory visual source of truth for the active primary page.
- Before the first prototype draft is created, bind the active module to `2-3` named mature product references that match the `competitive_reference_band`, then record which hierarchy, density, CTA, trust-signal, and copy-compression decisions are intentionally inherited from each one. Do not treat loose mood inspiration as sufficient reference evidence.
...
- When the confirmed primary page is trust-heavy, conversion-sensitive, premium-positioned, or first-impression-sensitive enough that a native static prototype alone would not prove commercial quality, open a Product Design-owned high-fidelity review prototype before accepting the native module prototype. Typical triggers include checkout, subscription, pricing, finance, health, security, identity, creator monetization, or any surface whose business outcome depends on immediate trust and perceived polish.
- The prototype must restore the confirmed module final effect image's static layout, hierarchy, spacing, visual grouping, and visible emphasis clearly enough for page confirmation before downstream architecture and implementation work.
- The prototype may keep placeholders only for necessary runtime-data or state-dependent content regions that are already declared in the active module contract. Decorative flourishes, textures, ornaments, and purely visual filler regions from the effect image must not be converted into placeholder blocks.
- When a necessary placeholder remains, keep it semantically named and bounded to the real content region it stands for. Do not use generic decorative boxes, helper mats, or ornamental placeholder cards merely to preserve composition.
...
- Review the prototype against the active module's page-content contract before treating it as ready. Use the prototype to confirm visibility order, grouping, default-surface density, copy volume, CTA dominance, and state-specific content differences, not to invent missing content policy from scratch.
...
- The confirmed native HTML prototype is static layout evidence. It does not replace `impl.md` for behavior definition, and it does not replace the module design source as the frozen implementation design file.
...
- If the module native HTML prototype is revised after review, treat that revision as a requirements change whenever it changes first-screen-visible content, deferred or collapsed content, default-surface exclusions, visible actions, wording, grouping, CTA priority, state presentation, or page structure. Update the active module `impl.md`, any affected shared design rules, and the workflow record before architecture, implementation, or any later visual-enhancement step continues.
...
- Before execution, the workflow must lock one `design_source_branch=stitch_design_source_branch|pencil_mcp_design_source_branch` for the active module, and must also record the matching `design_source_type`. When the selected branch is `stitch_design_source_branch`, it must additionally record both `stitch_project_mode=new_project|existing_project` and `design_source_project_ref`.
- When `design_source_branch=stitch_design_source_branch`, execution must use Stitch MCP with `model=GEMINI_3_1_PRO`, and the request must carry the recorded `stitch_project_mode` plus `design_source_project_ref`. If the project already exists, pass its recorded Stitch project `id` and continue in that project instead of creating a new one.
- When `design_source_branch=pencil_mcp_design_source_branch`, execution must use Pencil MCP instead of CLI or ad hoc file generation.
...
- The original accepted design source remains a prerequisite for the first-pass architecture and code path. If later visual-enhancement evidence is generated after all modules become functionally complete, run one additional focused reinforcement pass on the same selected tool instead of replacing the original design-source position.
- If that later reinforcement stays on `stitch_design_source_branch`, keep using Stitch MCP with `model=GEMINI_3_1_PRO` and pass the recorded `stitch_project_mode` plus `design_source_project_ref` into the reinforcement request. Do not switch from the recorded Stitch project to a newly created project unless the user explicitly reopens the project-binding decision.
...
## Design Restoration Boundary

The `design_restoration_ready` stage exists to turn the frozen design source into a Flutter restoration contract before architecture and code execution. If the later visual-enhancement path later produces stronger effect-image or atlas evidence, that path may trigger one additional restoration refresh on top of the same selected design-source tool.

- The workflow must route through `design-source-to-flutter-restoration`.
...
- Do not start architecture or code execution without this restoration contract once the selected design source has become the frozen primary design source. If a later visual-enhancement pass refreshes that design source, update the restoration contract before consuming the refreshed packet.
...
If `explanation_overload` appears, revise the design before HTML prototype acceptance, final effect-image acceptance, design-source acceptance, or freeze.
...
The selected direction must become one `fixed_style_direction`. Treat it as the `style_single_source` for all later module prototypes, module effect images, module design sources, and Flutter display restoration. The workflow must not mix visual families from multiple Mobbin references after the style direction is selected.
...
The window closes at the `freeze_boundary`: once the commercial direction is selected and `global_style_scheme.status=selected`, later module sketches, module effect images, design-source generation, and Flutter restoration must narrow around the confirmed style system instead of reopening broad layout exploration.

### Commercial Product Surface Gate

Every global style scheme, module HTML prototype, final effect image, and design source must produce a `commercial_surface_gate` receipt before it can be accepted, frozen, or used by a downstream stage.
...
- `artifact_type`: `shared_html_prototype`, `module_html_prototype`, `final_effect_image`, `design_source`, or `freeze_packet`
...
- `trust_signal_presence`: whether the surface exposes the right confidence cues for the product class, such as credible status, safety, pricing, permissions, identity, or result quality signals
- `premium_surface_score`: whether the artifact reads as commercially mature through restraint, hierarchy, density, spacing, and finish instead of decorative spectacle
...
The gate must fail when the artifact cannot plausibly sit beside the declared `competitive_reference_band`, when required trust signals are still absent for the product class, or when "premium" depends mainly on decorative imagery, oversized slogans, or negative-space theater instead of strong product-surface structure.

If `commercial_surface_gate.result=revision_required`, the workflow must route back to the matching artifact owner for one scope-matched revision pass. Do not promote the stage, freeze the design, enter atlas work, enter design-source work, or hand off to Flutter restoration until a later receipt records `commercial_surface_gate.result=passed`.
...
- optional design-prototype branch -> `Product Design:prototype` or `Product Design:image-to-code`
  Use only when the workflow explicitly needs a Product Design-owned coded prototype or visual-target implementation for design review, prototype sharing, or source-visual validation. Do not use these to replace the mandatory module native `HTML/CSS/JS` prototype gate, the design source, or Flutter implementation work.
...
`@product-design` may control design brief quality, direction quality, prototype quality, design QA quality, and the confirmed solid-atlas image generation pass, but it does not own PRD writing, Flutter technical baseline generation, module splitting, design-freeze approval, atlas background-removal, atlas slicing, design restoration, Flutter architecture, or final Flutter implementation.
...
- `references/asset-atlas-flow.md`: Read after design-source review whenever atlas-slice outputs or supplemental bitmap assets still require runtime-asset normalization.
...
- `@ui-sheet-atlas-slicer`: Use after the atlas bundle is confirmed and the workflow must cut the atlas through one generic config-driven slicing step before entering design restoration.
...
Every module final effect-image generation request must explicitly include existing image-backed design constraints: `art_direction`, `visual_system`, `cta_posture`, palette direction, typography mood, component family cues, image-treatment posture, and the approved reference-image baseline when those values already exist. Those constraints must be read from the current frozen global-design packet first, then narrowed only by the active page or module needs; if the global packet cannot supply the required visual constraints, stop and repair the packet instead of generating a speculative image. Every request must also carry the frozen design width plus the frozen base viewport height as the minimum logical output size, and must separately carry `image_output_scale` to derive the workflow output size. For example, a frozen viewport of `390 x 844` with `image_output_scale=2x` means the generated image should start from about `780 x 1688` pixels before any intentional extra scroll height. When the design target includes a scrollable page or scrollable region, allow a taller output height to show the designed scroll range, but do not shrink width or reduce height below the frozen minimum in order to fit more content into one screen. Final effect-image requests must go through `Product Design:ideate`, and any revision request must inherit the same constraints plus the accepted review notes before the Product Design-owned final effect-image direction pass is frozen as the selected baseline. The workflow must select or revise a Product Design-generated candidate instead of generating the module final effect image through `gpt-image-2-generator`. The module stage must stay inside the confirmed `@product-design` brief and must target mature commercial product UI/UX rather than speculative concept art, wireframe-like draft output, marketing illustration, ad visual, or slogan-led hero art. The generation prompt must explicitly say that the output is an in-product commercial app screen, with functional controls, believable information density, and product-task hierarchy as the first priority. When the effect image is confirmed, the next native HTML prototype must preserve that structure first and only keep necessary runtime-content placeholders; decorative regions must not be turned into placeholder blocks. If the selected final effect-image direction path is blocked by missing access, credentials, or upstream capability, stop and record a blocker for the workflow. All workflow final effect images must use light mode unless the user explicitly changes that requirement upstream.

For module implementation, the first priority before freeze is faithful restoration of the confirmed effect-image-backed layout structure, task path, action order, and interaction ownership, with functional completeness and interaction experience taking priority over later-stage asset expansion. Before queueing or applying `module_design_frozen`, verify that the global design freeze is already complete, that the module `impl.md` already fixed the module function, the primary interaction path, non-display behavior boundaries, and platform-aware information-density decisions under the frozen shared design and interaction principles, that the module final effect image has already been generated and accepted for the active module, that a native static HTML module prototype has been generated and accepted as a restoration of that effect image, and that the display restoration blueprint is already strong enough to support a first-pass implementation using Flutter SDK standard capabilities plus platform HIG-aligned interaction constraints. The default freeze scope is the confirmed primary page only; any additional page states must be appended later only after explicit human confirmation. Atlas extraction analysis, atlas evidence, display evidence packs, module design source, and bitmap assets belong to conditional follow-up evidence only when they are still necessary after the confirmed effect image and restoration review have already fixed the visual baseline. If the initial implementation inputs are missing, vague, or weaker than the implementation target requires, keep the module unfrozen and route back to the correct scope-matched revision. After the module has entered implementation, prefer standard-library-first restoration and postpone bitmap-dependent refinement decisions until the confirmed effect image plus implementation review explicitly prove they are needed.
...
Do not move a module into `implementing` until `technical_baseline_ready`, `module_impl_docs_ready`, `module_design_frozen`, `impl_rd_ready`, `design_restoration_ready`, `display_restoration_blueprint_ready`, and `bootstrap_code_ready` exist for the module, confirmed maturity is at least `impl_status=landed` and `design_source_status=frozen`, the frozen tool-selected structured design-source packet exists for the module, the display evidence pack plus display-layer decision table are already complete, and the required global public code baseline is already landed.
