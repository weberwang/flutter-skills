...
The `shared_html_prototype_ready` stage exists to make the shared/global product shell and shared page posture reviewable in native web form before any shared/global visual reference image generation begins.

- Enter this stage after the technical baseline, design-device preset, Product Design brief, and shared direction inputs are ready enough to express a concrete shared shell and shared page structure.
- The shared prototype must show the shared shell, first-screen hierarchy, key shared regions, spacing rhythm, and page grouping clearly enough for confirmation before representative sketch or final effect-image work starts.
- The shared prototype must use only native `HTML`, `CSS`, and `JavaScript`. Do not use any framework, component runtime, generated SPA shell, or design-tool-specific app scaffold.
