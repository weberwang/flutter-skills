...
Route a Flutter module through rough idea intake -> idea-sketch brainstorming -> requirements brainstorming -> PRD document generation -> global technical baseline -> target design-device preset, design viewport, and image output scale confirmation -> `@product-design` brief confirmation -> Mobbin-backed global theme and style scheme selection -> `DESIGN.md` output for theme, hierarchy, public shell, component family, and interaction principles only -> shared freeze -> early project initialization plus bootstrap preparation -> module `impl.md` generation under frozen design and interaction principles -> mandatory module final effect-image confirmation -> mandatory native HTML module prototype restoration from that confirmed effect image -> mandatory module Pencil design source confirmation -> implementation RD readiness -> architecture -> display restoration blueprint readiness -> implementation workflow -> Flutter render versus frozen-design QA -> `@product-design` design QA -> human visual inspection -> later atlas preparation, atlas slicing, Pencil reinforcement, and bitmap assets only when the confirmed effect image plus implementation review still prove they are necessary.
...
- Required module-stage artifacts:
  for the initial implementation pass, the required path is the module `impl.md`, confirmed module final effect image, confirmed native `HTML/CSS/JS` prototype restored from that effect image, mandatory Pencil design source, architecture output, and display restoration blueprint. Atlas and standalone bitmap generation remain conditional follow-up evidence only when the confirmed effect image plus implementation review still show regions that code restoration alone cannot cover faithfully enough.
...
The module final effect-image stage exists before native prototype confirmation. It is the required high-fidelity visual baseline for the active module's confirmed primary page and must be generated and accepted before the workflow asks native `HTML/CSS/JS` to restore that page.

- Use `Product Design:ideate` to generate the module final effect image after the active module `impl.md` already fixes function, the primary task path, and non-display behavior boundaries for the confirmed primary page.
...
- Treat the final effect image as the pre-prototype visual baseline for later HTML, Pencil, architecture, and Flutter restoration. Do not treat it as a late polish branch.
...
- Enter this stage only after the shared/global design freeze is complete, the active module `impl.md` already fixes the module function, the primary task path, and non-display behavior boundaries for the confirmed primary page, and the matching module final effect image has already been explicitly confirmed.
