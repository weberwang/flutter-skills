...
The module final effect-image stage exists to turn the confirmed native HTML structure into the selected light-mode visual baseline for downstream atlas, Pencil, freeze, and implementation work.

- Use `Product Design:ideate` to generate the module final effect image so the page's product temperament, information hierarchy, module order, CTA posture, shell fit, and fit with the already-frozen visual system are all judged under the Product Design controller.
...
- Treat the final effect image as the reviewable module visual baseline. It should be strong enough to answer both "is this the right page?" and "is this the right visual world?" before atlas extraction and Pencil work continue.
