...
description: Use when coordinating a Flutter product workflow across idea intake, requirements, PRD, technical baseline, Product Design brief, module responsibility confirmation, reference-image effect images, UI atlas preparation, Pencil design source, shared/module design freeze, module impl docs, native HTML prototype confirmation, Pencil-to-Flutter restoration, architecture, display restoration blueprint, implementation readiness, QA, human visual inspection, workflow state, or when the user asks what stage should happen next.
---
...
- Required module-stage artifacts:
  for the initial implementation pass, the required path is confirmed module responsibility and primary-page scope, confirmed module final effect image, executable module `impl.md`, confirmed native `HTML/CSS/JS` prototype restored from that effect image, mandatory Pencil design source, architecture output, and display restoration blueprint. Atlas and standalone bitmap generation remain conditional follow-up evidence only when the first-pass implementation plus implementation review still show regions that code restoration alone cannot cover faithfully enough.
...
- Treat the final effect image as the pre-implementation visual baseline for later `impl.md`, HTML, Pencil, architecture, and Flutter restoration. Do not treat it as a late polish branch.
- The native HTML prototype, later Pencil design, and later Flutter display restoration must inherit the confirmed effect image's page structure, hierarchy, grouping, density, action order, and visible emphasis unless the workflow explicitly reopens the image direction.
...
- Atlas preparation, slicing, Pencil reinforcement, and standalone bitmap generation remain later follow-up work only when the confirmed effect image and restoration review still prove they are necessary.
