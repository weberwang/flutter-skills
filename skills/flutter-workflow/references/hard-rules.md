...
- Do not let `--auto` or `--full-auto` generate required module final effect images without `gpt-image-2-generator` being available for that branch, and do not skip the local module representative-sketch stage through `$imagegen` before that final generation when the module route requires it.
