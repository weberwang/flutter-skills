...
- Do not treat module implementation as `display first, behavior later`; the required local order is `impl -> final effect image -> native HTML prototype -> atlas analysis -> atlas preparation -> atlas background removal -> atlas slicing -> Pencil design -> non-display code -> display restoration`.
