...
- do not route shared/global scope into shared Pencil design execution or shared page image-asset generation; stop the global route at confirmed `global_style_scheme` plus `DESIGN.md`, and keep page-level design-source work in module scope only
...
7. Run the active module's first-pass freeze preparation first, freezing the design-source packet only after confirmed module scope, confirmed effect image, executable `impl.md`, confirmed native HTML prototype, and original mandatory Pencil design source are all explicit enough. Only after that freeze and after first-pass implementation is complete may the workflow open a later visual-enhancement loop when needed.
