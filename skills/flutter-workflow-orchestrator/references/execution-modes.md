...
- route into mandatory shared Pencil design execution before shared freeze, then into post-review shared image-asset generation only when still required, then into the shared `prototype_required_decision`, and enter shared HTML interactive prototype generation only when that decision explicitly requires it; do not generate final module page implementation code at global scope
...
- treat every generated module `impl.md` as a detailed module task implementation document constrained by the frozen shared design and interaction principles, the confirmed Product Design clarification packet, and the target platform's information-density expectations, and strong enough to drive a later implementation-stage visual restoration loop covering the active module's effect-image generation, display-evidence-pack confirmation, mandatory module Pencil design execution, post-review module image-asset generation when needed, and optional module HTML interactive prototype generation; separate refinement documents are not required
- freeze each active module's contract before implementation-side visual restoration begins; module effect images, module display evidence, and any required module HTML interactive prototype work now belong to that later implementation-stage loop rather than the pre-implementation freeze gate
