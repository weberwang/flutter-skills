...
- User says "先让子代理给推荐方案，再由我拍板": allow. Route to the recommendation branch, wait at `design_recommendation_ready`, then require human confirmation before promoting `product_direction_confirmed`.
...
- User says "run `flutter-workflow-orchestrator --auto`": keep advancing through technical baseline, executable module `impl.md` generation, global visual design brainstorming, direct confirmation or recommendation-then-confirmation, `DESIGN.md` output, optional effect-image evidence only when the active path needs it, selected structured design-source generation, shared freeze, module freeze, implementation-readiness preparation, architecture, project initialization, and bootstrap code generation for every dependency-safe target module until all target modules are waiting at the implementation boundary, then stop before code.
...
- User says "the code is done, check whether it matches the design": stop and request human visual inspection against the frozen selected structured design source and any approved page effect images; do not create a separate automatic review workflow stage.
