...
- User says "确认了设计方向就直接进 HTML 原型，不写 DESIGN.md": block. Final product direction confirmation must be followed by `DESIGN.md` output before any HTML interactive prototype branch.
...
- User says "generate all page effect images now" before the representative image is confirmed: block. Generate one representative effect image first, wait for user confirmation or revision feedback, then generate the remaining required pages.
...
- User says "run `--auto` and keep going after the first effect image": allow when the active route explicitly requires additional effect-image evidence. `--auto` should keep generating the remaining in-scope effect images without stopping for confirmation.
- User says "skip global effect images and freeze directly": block. The representative effect image is mandatory before shared freeze.
...
- User says "use HTML prototype as design source": allow. Keep `DESIGN.md` as the upstream constraint packet and use any approved effect images as the primary visual baseline when they exist.
- User says "generate HTML prototype for all pages" before shared/global freeze passes: block. Shared/global freeze may only produce shared theme and public-shell design, not final module page implementation code.
- User says "generate HTML prototype for all pages" after shared/global freeze passes: allow only inside the active module path, run page-scoped prototype work in subagents with at most 6 parallel page designs, and block if a subagent tries to own workflow state or more than one page in the same batch.
...
- User says "let each prototype page decide its own style": block. Page subagents must expand pages from one frozen shared design master packet and may not redefine global style locally.
- User says "参考工具自己的内部指导来补设计细节": block. Design guidance may come only from the frozen global design, the active module design package, the frozen HTML prototype artifacts, and `DESIGN.md`.
- User says "this page needs a better component, just change it inside the prototype": if the change affects cross-page component families or shell rules, route back to orchestrator-owned shared-packet revision instead of allowing page-local redesign.
...
- User says "run `flutter-workflow-orchestrator --auto`": keep advancing through technical baseline, `@product-design` brief confirmation, representative effect-image generation from reference images, final direction confirmation, `DESIGN.md` output, shared HTML interactive prototype generation, shared freeze, then enter the serial per-module loop of `impl.md` generation -> module effect image -> module HTML interactive prototype -> module freeze -> implementation-readiness preparation -> architecture -> `@superpowers` `Spec` -> `@superpowers` `Plan` -> code implementation, followed by project initialization and bootstrap code generation where needed, until all target modules are fully implemented or a blocker appears.
...
- User says "the code is done, check whether it matches the design": first run `@product-design` `design-qa` when a source visual target and implementation screenshots both exist, then request human visual inspection against the frozen prototype-derived design source and any approved page effect images; do not create a separate automatic review workflow stage.
