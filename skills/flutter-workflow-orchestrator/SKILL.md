...
- The resource generation unit is always one approved bitmap resource. A shared resource lives under `docs/project/assets/shared/<page-name>/`, and a module resource lives under `docs/project/modules/<module>/assets/<page-name>/`. The matching atlas artifacts that prepare those resources live separately as `UI-only sheet atlas` evidence and do not replace the final per-resource output files.
...
- Every new bitmap resource must be generated through `$imagegen` as its own prompt and saved as its own file. Do not pack multiple final runtime assets into one atlas image and do not use TexturePacker-style intermediate contracts. A matching `UI-only sheet atlas` may exist as a transparent pre-cut baseline, but it must still stay separate from the final per-resource outputs.
...
- Background processing is forbidden for already transparent atlas or slice inputs. Only solid-color backgrounds may be processed to transparent; non-solid backgrounds must not enter background removal.
...
- `references/asset-atlas-flow.md`: Read after Pencil design review whenever non-standard bitmap assets may still be required, and treat it as the single-resource generation flow.
