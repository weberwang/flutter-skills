...
description: Use when coordinating rough Flutter idea intake, idea-sketch brainstorming, requirements brainstorming, PRD document generation, technical baseline generation, Product Design brief confirmation, reference-image-backed effect-image generation, design-direction confirmation, DESIGN.md output, mandatory Pencil design-source generation, post-review shared image-asset generation, optional HTML interactive prototype design source generation, shared design freezing, module implementation document generation, module design freezing, implementation-stage module effect-image generation, display-evidence-pack checks, implementation-stage post-review module image-asset generation, Pencil-to-Flutter restoration planning, architecture planning, display restoration blueprint generation, implementation readiness, Product Design design QA, human visual inspection handoff, workflow state, or when a user asks what stage should happen next.
---
...
Route a Flutter module through rough idea intake -> idea-sketch brainstorming -> requirements brainstorming -> PRD document generation -> global technical baseline -> target design-device preset and base resolution confirmation -> `@product-design` brief confirmation -> reference-image-backed representative sketch generation through local `$imagegen` under `@product-design` direction -> final product design direction confirmation from the approved sketch -> representative final effect-image generation through `gpt-image-2-generator` under the same `@product-design` constraints -> `DESIGN.md` output -> mandatory shared Pencil design-source generation -> shared image-asset generation after review when still needed -> optional HTML decision for the shared scope -> optional post-direction Creative Production asset production / polish when the request is asset-oriented -> optional shared HTML interactive prototype design source -> shared freeze -> early project initialization plus bootstrap preparation -> module `impl.md` generation under frozen design and interaction principles -> module design freeze -> implementation RD readiness -> implementation-stage module effect-image generation -> display-evidence-pack confirmation -> mandatory module Pencil design-source generation -> module image-asset generation after review when still needed -> optional HTML decision for the active module -> optional module HTML interactive prototype generation -> Pencil-to-Flutter restoration planning -> architecture -> display restoration blueprint readiness -> implementation workflow -> Flutter render versus frozen-design QA -> `@product-design` design QA -> human visual inspection.
...
The required design-source output is a frozen structured restoration packet whose primary design-file form is now Pencil, not HTML. After the final design direction is confirmed and `DESIGN.md` is written, the workflow must convert the approved screenshot references, the confirmed representative final effect-image baseline, and `DESIGN.md` into one shared structured design packet, then build a high-fidelity Pencil design source through MCP. HTML is now an optional supplemental source only. Use the HTML interactive prototype flow only when structure semantics such as scroll ownership, sticky behavior, overlays, cross-state layout transitions, or interaction-region ownership still cannot be restored reliably enough from the frozen Pencil design source, `DESIGN.md`, frozen effect images, any post-review confirmed generated asset files, and the active implementation contracts alone. If HTML is not required, do not block freeze on prototype production. `@product-design` still owns the design-side kickoff: brief confirmation, direction framing, visual-target selection, and the commercial product UI/UX bar for both sketch and final-image stages. The shared/global flow must first generate one local representative sketch through `$imagegen` under `@product-design` constraints, wait for explicit sketch confirmation in manual mode, and only then generate the representative final effect image through `gpt-image-2-generator` under the same accepted constraints. Final effect images remain mandatory, but the workflow now requires only the representative final effect image at the shared/global stage. The orchestrator must first route through `effect-image-to-pencil-design` to build the mandatory Pencil design source, compare the result against the approved effect image, output a visible-difference list plus concrete repair actions, stop for confirmation, apply one focused repair pass, and only then request human acceptance of the Pencil design file. Only after that review result is confirmed may the workflow decide whether any non-standard bitmap assets are still required and route through post-review shared image-asset generation. Separate module design documents must not replace the frozen structured design-source packet. All module-related work must stay behind the global design freeze. Only after the shared design direction, shared interaction principles, shared public shell rules, and shared component families are frozen may the workflow start the per-module loop. In that loop, the active module's `impl.md` becomes a detailed module task implementation document constrained by the frozen design and interaction principles. The module no longer needs an independent design-draft stage. Instead, after `impl.md` fixes the module function, key states, task path, and non-display behavior boundaries, the workflow should freeze the module design contract first, then run module-scoped effect-image generation, display-evidence-pack confirmation, mandatory module Pencil design-source generation, post-review module image-asset generation, and any optional module HTML supplementation inside the implementation-stage visual restoration loop. If HTML is required, module-scoped page prototype work belongs only to the active module after the shared/global freeze has passed and the active module's `impl.md` is already implementation-final. Before every shared/global or implementation-stage module sketch or effect-image generation request, the workflow must first read the frozen global design packet and explicitly extract the visual constraints that already exist for that page or module, including palette direction, typography mood, component family, CTA posture, image-treatment posture, spacing or hierarchy bias, and any other approved visual-system rules; both sketch generation and final effect-image generation may only extend those constraints rather than inventing replacements. At both stages, generated output must read as mature commercial product UI/UX rather than a requirements mock, poster, or explanatory collage. At the shared/global freeze scope, any selected HTML interactive prototype packet may express or validate only the shared theme system, shared public shell, shared public component families, and shared interaction principles; it must not generate final module page implementation code at that stage. When HTML is selected, the prototype flow must treat the frozen final effect-image baseline, the frozen Pencil design source, and approved implementation-stage module effect images as the primary visual baselines and `DESIGN.md` as the constraint packet rather than replacing the visual baseline with prose. Design guidance must come only from the frozen global design images, the frozen Pencil design source, the active module design-source packet, uploaded approved screenshot references, any selected frozen HTML prototype artifacts, any post-review confirmed generated asset files, and `DESIGN.md`; tool-native defaults, hidden heuristics, or internal style opinions are not valid guidance sources. After any shared/global or module freeze is confirmed, delete previously generated but no-longer-selected sketches or effect images from the corresponding workflow artifact directories and indexes so later routing cannot accidentally reuse stale alternatives. Restored design output must preserve one shared style direction, one shared theme system, one shared public shell contract, one shared public component family contract, and one shared interaction-principle contract across all pages. Page-scoped prototype runs stay capped at at most 6 parallel page-design subagents, and only the orchestrator may merge receipts, update the workflow record, or freeze the packet.
...
- The asset catalog is also the mandatory generated-image record table. Every workflow-generated representative sketch, final effect image, implementation-stage module effect image, shared bitmap asset, and module bitmap asset that enters workflow evidence or downstream consumption must be written into that catalog in the same workflow step.
...
- The resource generation unit is always one approved bitmap resource. A shared resource lives under `docs/project/assets/shared/<page-name>/`, and a module resource lives under `docs/project/modules/<module>/assets/<page-name>/`.
...
- Every new bitmap resource must be generated through `$imagegen` as its own prompt and saved as its own file. Do not pack multiple assets into one atlas image and do not use TexturePacker-style intermediate contracts.
- Generated image assets must decide their background mode from the frozen design intent. Use transparent background when the asset should float over runtime surfaces or multiple backgrounds, and keep a baked background only when the frozen design explicitly treats that background as part of the asset itself.
...
- Shared and module structured design sources, including HTML prototypes when they are selected, must directly reference the final generated asset files for every approved bitmap asset. Regions that are explicitly tracked as `placeholder_only` may remain placeholders, but screenshot crops or undeclared temporary placeholder assets are not allowed.
...
- The display evidence pack must be confirmed before the workflow finalizes `prototype_required_decision`, module freeze, architecture mapping, or display-layer readiness.
...
- Do not skip Pencil and move directly from effect image to freeze, HTML prototype, architecture, or code.

## Prototype Required Decision Boundary

The `prototype_required_decision` stage exists to decide whether the current shared or module scope still needs an HTML interactive prototype after Pencil is already frozen.

- HTML is required only when the frozen Pencil design source plus the frozen packet still lack enough structure semantics to restore the page reliably in Flutter, such as complex scroll ownership, sticky regions, overlay stacks, tab or pager ownership, or major cross-state layout transitions.
- HTML is not required when the frozen Pencil design source, `DESIGN.md`, effect images, confirmed generated asset files, `impl.md`, and the shared freeze rules already define the structure strongly enough for downstream architecture and the later `display_restoration_blueprint`.
- Whether the answer is `required` or `not_required`, write the reason explicitly into the workflow record before freeze. Do not leave HTML as an implicit default.
- If HTML is required, block freeze until the corresponding prototype-derived packet is generated or validated.
- If HTML is not required, block freeze until the fallback structured design-source packet explicitly names the artifacts that will replace the prototype's structural role.

Default HTML prototype stack policy, when HTML is selected: use `Vite + React + TypeScript` unless the user explicitly overrides it. This default exists because it balances fast prototype implementation with stable Flutter restoration: React component structure is easier to translate into region ownership, state slots, overlay semantics, and the later `display_restoration_blueprint`. Do not default to Vue for the shared workflow unless a project-specific constraint requires it, because it adds a second component mental model without improving Flutter restoration enough to justify the drift. Do not default to plain `HTML/CSS/JS`, because it is easy to start but weak as a frozen design source: structure, interaction semantics, and state ownership are too easy to scatter across selectors and ad hoc scripts, which makes Flutter restoration noisier and less reliable.
...
- The blueprint remains mandatory whether the upstream design source came from an HTML prototype or from a non-HTML frozen structured packet.
...
- Compare Flutter output against the frozen design packet, including any selected HTML prototype artifacts, approved effect images, confirmed generated asset files, and the frozen `display_restoration_blueprint`.
...
- `references/asset-atlas-flow.md`: Read after Pencil design review and before the shared or module `prototype_required_decision` whenever non-standard bitmap assets may still be required, and treat it as the single-resource generation flow.
...
For module implementation, high-fidelity visual fidelity is still the first design-freeze priority, but the pre-implementation freeze now validates contract readiness rather than demanding module-scoped image artifacts up front. Before queueing or applying `module_design_frozen`, verify that the global design freeze is already complete and that the module `impl.md` already fixed the module function, states, interaction path, non-display behavior boundaries, and platform-aware information-density decisions under the frozen shared design and interaction principles strongly enough to drive a later implementation-stage visual restoration loop. If the shared constraints or function contract are missing, vague, or weaker than the implementation target requires, keep the module unfrozen and route back to the correct scope-matched revision. After the module has entered implementation, require the display evidence pack, `prototype_required_decision`, implementation-stage module effect image, and selected structured design-source packet before later display restoration decisions are finalized.
