...
- If an atlas or slice input is already transparent, do not run transparency repair again. When a transparency repair is still required, use prompt text exactly `移除背景` through `/images/generations` and pass the current image through `image_urls` as base64 data-URI input.
...
- `@effect-image-to-ui-sheet-atlas`: Use when the workflow must generate the final effect image together with a UI-only atlas, atlas manifest, and atlas slicing config as one reviewable node.
