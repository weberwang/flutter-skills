...
- 新增包前，先说明它属于哪个能力域、由哪一层拥有、放在哪个 feature 使用。
...
- 真正写入项目时，优先用与当前 Flutter SDK 兼容的最新版，并在需要时去官方文档或 pub.dev 核实
- 如果最新版与当前 Flutter SDK 不兼容，向下选择到最近的兼容版本，而不是保留过时且未经验证的旧版本
- 包或插件接入后必须执行验证；如果出现依赖解析、生成代码、编译或分析不兼容问题，必须继续修复直到通过验证
