...
- UI tasks must link a complete page responsive contract using `flutter-ux-ui-quality/references/responsive-layout-strategy.md` version `1.1`. The brief records region-tree and region-to-primitive mapping coverage, structural breakpoint triggers, allowed overlay whitelist, prohibited coordinate translation, concrete viewport matrix, large-text/long-content/keyboard/SafeArea evidence, and Widget/Golden/audit commands. A missing contract or single image returns `NEEDS_CONTEXT`.
