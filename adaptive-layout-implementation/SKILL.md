...
1. 从已审页面语义合同复制 [sketch 模板](assets/layout-spec-template.yaml)，填写区域、锚点、断点、内容容器、系统避让、滚动 owner、文本增长、叠层、实现约束、不变量和 `evidence_matrix`。
2. sketch 阶段禁止写 `critical_alignments` 或 `parity_cases`，避免伪造未来证据。运行 `python scripts/validate_layout_spec.py <layout-spec.yaml>` 后才实现 Code Sketch。
3. 在生产 Flutter 骨架中实现并执行实际 Widget 关系测试。结构门禁通过后才按风险生成截图；截图不能替代关系测试。
